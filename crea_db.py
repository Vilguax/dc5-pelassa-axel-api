...
conn = sqlite3.connect('kanaps.db')
c = conn.cursor()

c.execute('''
    CREATE TABLE IF NOT EXISTS kanaps (
...
for item in data:
    c.execute('''
        INSERT INTO kanaps (_id, name, price, imageUrl, description, altTxt, colors)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (item['_id'], item['name'], item['price'], item['imageUrl'], item['description'], item['altTxt'], ','.join(item['colors'])))
